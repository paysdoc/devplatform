...
- Runners: `ubuntu-latest` for all jobs; `oven-sh/setup-bun@v2` for all; `actions/setup-node@v4` additionally for `package`, `release-dry-run`, and `release.yml` (all Node 22 — Cucumber 13 refuses to start on Node 20, which is what the `@packaging` BDD step in `package` hit on every run until it was bumped).
...
- The npm trusted publisher for `@paysdoc/devplatform` must name GitHub Actions, owner `paysdoc`, repository `devplatform`, workflow filename `release.yml`, no environment, **and have the direct `npm publish` action allowed** — npm creates it with `npm stage publish` permission only. Missing that tick is what `403 … OIDC permission denied for this action` at the publish `PUT` means, after the OIDC exchange in `verifyConditions` has already succeeded; a linkage mismatch fails earlier, at the exchange. Do not fall back to `NPM_TOKEN` for this (bypass-2FA tokens are retired in January 2027; see `app_docs/feature-9xqejz-release-automation.md`). If a run tags a version and then fails to publish it, delete the orphan tag (`git push origin --delete vX.Y.Z`, plus any GitHub release) and re-run via `workflow_dispatch` — never delete `v1.0.0`, the baseline guard hard-fails without it.
