...
- The npm trusted publisher for `@paysdoc/devplatform` must name GitHub Actions, owner `paysdoc`, repository `devplatform`, workflow filename `release.yml`, and no environment; the package's Publishing access setting must also permit trusted publishing. Run 34503029087 saw the OIDC token exchange succeed in `verifyConditions` and then the publish `PUT` itself denied (`403 … OIDC permission denied for this action`) — a distinct, later failure mode from a missing/misconfigured trusted-publisher entry. `NPM_TOKEN` is the fallback (see `app_docs/feature-9xqejz-release-automation.md`). If a run tags a version and then fails to publish it, delete the orphan tag (`git push origin :refs/tags/vX.Y.Z`, plus any GitHub release) and re-run via `workflow_dispatch` — never delete `v1.0.0`, the baseline guard hard-fails without it.
