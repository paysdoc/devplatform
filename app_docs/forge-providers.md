...
- Since issue #11, the GitHub barrel (`src/providers/github/index.ts`) re-exports the GitHub-named runtime helpers (`createGitHubTokenProvider`, `resolveContextToken`, `getInstallationToken`, `isGitHubAppConfigured`, `resolveBootstrapGitIdentity`, `ghAuthToken`) so ADW's launch boundary can switch to `@paysdoc/devplatform/providers` with no behaviour change; `createForgeCredentials` remains the forge-neutral, recommended route and composes exactly those functions. The public barrel (`index.ts`, this module) still re-exports only `GitHubAppConfig` and `AppAuthDeps` as types — the GitHub barrel must not re-export them too, since `index.ts`'s `export *` over both barrels would collide (TS2308) — and still carries no GitLab-named runtime helper (`createGitLabTokenProvider`/`resolveGitLabBootstrapGitIdentity` stay deep-import only). `createForgeCredentials` remains the only new public runtime name this module itself adds beyond `forgeProviders`.
