...
- Run 34503029087 tagged `v1.1.0` and then failed at `npm publish` with `403 Forbidden … OIDC permission denied for this action` — the OIDC token exchange itself had already succeeded, so this is a distinct failure from a missing trusted-publisher entry (check the package's Publishing access setting on npmjs.com, and that the account that created the trusted publisher holds publish rights). `v1.1.0` is tagged on `origin` but was never published; the next successful `feat`-carrying release supersedes it (e.g. `v1.2.0` contains everything `v1.1.0` would have, so `v1.1.0` never needs publishing separately). A run that fails after tagging leaves an orphan tag a re-run will not republish, since semantic-release finds no new commits after the tag — delete the orphan tag and any GitHub release it created, then trigger `workflow_dispatch` again.
