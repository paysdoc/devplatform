...
- `403 Forbidden … OIDC permission denied for this action` at `npm publish`, *after* the OIDC token exchange succeeded and provenance was signed, means the npm trusted publisher lacks the direct-publish action. npm creates a trusted publisher with `npm stage publish` permission only; "allow `npm publish`" is a separate tick that is off by default. This is not a trusted-publisher linkage problem (a mismatch fails earlier, at the exchange, with a 404) and not the package's "disallow tokens" setting (that only affects classic tokens). Fix: tick the action on npmjs.com, or `npm trust github @paysdoc/devplatform --file release.yml --repo paysdoc/devplatform --allow-publish --allow-stage-publish`. Every release from run 34503029087 (2026-09-10) through 35720692819 (2026-09-22) failed this way; run 35720994056 published `1.1.0` once the action was allowed.
- Do not use `NPM_TOKEN` as the fix for the above. npm retires direct publishing via bypass-2FA tokens in January 2027, and a stage-only token cannot serve `@semantic-release/npm`, which only runs plain `npm publish` (staged publishing is an open request, semantic-release/npm#1160, that the maintainers have said needs an architectural change).
- semantic-release pushes the tag *before* `@semantic-release/npm` publishes, so a run that fails at `npm publish` leaves an orphan tag a re-run will not republish, since semantic-release then finds no new commits and reports "no relevant changes" — delete the orphan tag (`git push origin --delete vX.Y.Z`) and any GitHub release it created, then trigger `workflow_dispatch` again. Never delete `v1.0.0`.
- After a successful publish the registry metadata (`npm view`) shows the new version within about two minutes, but the tarball URL can return 404 for several minutes more (about 4½ minutes for `1.1.0`). That is npm replication lag, not a failed release.
